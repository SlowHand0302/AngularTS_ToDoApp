...
export const taskReducer = createReducer<TaskState>(
    ...
    on(TaskActions.loadTasks.success, (state, { tasks }) => ({
        ...state,
        tasks,
        loading: new Set([...Array.from(state.loading).filter((item) => item !== 'loadTasks')]),
    })),
    ...
    on(TaskActions.loadTaskByID.success, (state, { task }) => ({
        ...state,
        selectedTask: task,
        loading: new Set([...Array.from(state.loading).filter((item) => item !== 'loadTaskByID')]),
    })),
    ...
    on(TaskActions.loadTasksByUserID.success, (state, { tasks }) => ({
        ...state,
        tasks,
        loading: new Set([...Array.from(state.loading).filter((item) => item !== 'loadTasksByUserID')]),
    })),
    ...
    on(TaskActions.createTask.success, (state, { task }) => ({
        ...
        loading: new Set([...Array.from(state.loading).filter((item) => item !== 'createTask')]),
    })),
    ...
    on(TaskActions.updateTask.success, (state, { task }) => ({
        ...
        loading: new Set([...Array.from(state.loading).filter((item) => item !== 'updateTask')]),
    })),
    ...
    on(TaskActions.deleteTask.success, (state, { task }) => ({
        ...
        loading: new Set([...Array.from(state.loading).filter((item) => item !== 'deleteTask')]),
    })),
